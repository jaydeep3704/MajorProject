...
export function LoginForm() {
    return (
        ...
        <form action={async()=>{
            ...
            <GeneralSubmitButton text="Signin With Google" icon={<Google/>} variant={"secondary"} className="w-full" />
        </form>
        <form action={async()=>{
            "use server"
            await signIn("github",{redirectTo:"/"})
        }}>
            <GeneralSubmitButton text="Signin With Github" icon={<GitHub/>} variant={"secondary"} className="w-full" />
        </form>
        </CardContent>
    )
}